...
function logObjectWithID(arr) {
  for (let i = 0; i < arr.length; i++) {
    let flightWithScale;
    arr[i].scale ? flightWithScale = 'yes' : flightWithScale = 'no';
    console.log(`Flight ID ${arr[i].id}. From ${arr[i].from} to ${flights[i].to}. Cost ${arr[i].cost} €. Scales: ${flightWithScale}`);
  }
}
function logObjectWithoutID(arr) {
  for (let i = 0; i < arr.length; i++) {
    let flightWithScale;
    arr[i].scale ? flightWithScale = 'it has scales' : flightWithScale = 'it is a non-stop flight';
    console.log(`The flight from ${arr[i].from} to ${flights[i].to} costs ${arr[i].cost} € and ${flightWithScale}`);
  }
}
...
if (isAdmin) {
  do {
    console.log("Current list of flights:");
    logObjectWithID(flights); //Show list of flight to have a preview of it.
  ...
  do {
    ...
    let customizedPriceFlightsArr = flights.filter(flight => flight.cost <= maximunFlightPrice); //Flight puede ser sustituido por cualquier nombre, el método sabe que se refiere a cada item del array flights
    
    logObjectWithoutID(customizedPriceFlightsArr);
    
  } while (confirm("Filter by price again?"))
}
